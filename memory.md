...
|---|---|---|---|---|
| MEDIUM | XmlRunSettingsUtilities.cs | `ReaderSettings` property creates new `XmlReaderSettings` on every call (~14 call sites). Should be `static readonly`. XmlReader.Create still clones it, so savings = 14 source-object allocs/run | <1% | Not actionable as standalone PR |
| MEDIUM | TestRequestManager.cs:736-745 | `UpdateRunSettingsIfRequired` triples XML parse: creates `XmlDocument` then calls `GetRunConfigurationNode` + `GetLoggerRunSettings` (each re-parse full XML). Could consolidate via `XPathNavigator` overloads | 1-3ms/run | Need ≥30ms to hit 15% bar |
| LOW | TestPluginCache.cs:90-101 | `EqtTrace.Verbose` calls with eager `string.Join` — runs even when logging disabled | marginal | Fix only if bundled |
...
**Backlog cursor:** All items scanned. Full codebase scan completed 2026-07-07. Next scan should look at testhost startup path more deeply and DataCollector initialization.

## Tasks Last Run
- Task 4 (Maintain PRs): 2026-07-07 18:30 UTC — verified #16213/#16216/#16222 closed; #16210 open/green
- Task 2 (Identify opportunities): 2026-07-07 18:30 UTC — deep scan, no HIGH items found under new rules
- Task 7 (Monthly summary): 2026-07-07 18:30 UTC — updated #16211
